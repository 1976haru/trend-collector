...
`MOJHeader` 는 부처 마크가 있을 경우 다음 경로의 이미지를 표시하고, 없으면 자동으로 태극 풍 fallback 으로 동작합니다.

```
public/assets/moj-logo.png
```

자세한 안내는 [`public/assets/README.md`](public/assets/README.md) 참고.
외부 사이트 (예: 법무부 공식 홈페이지) 의 이미지를 직접 hot-link 로 가져오지 마세요. 부서 내부에서 적법하게 확보한 파일을 위 경로에 배치하시기 바랍니다.
