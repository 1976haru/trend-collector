...
// 비밀번호는 화면에 표시되지 않음. 빈 값으로 저장하면 기존 비밀번호 유지.
// ─────────────────────────────────────────────
...
export default function MailSettings() {
  ...
  const [form, setForm]         = useState({
    ...
    user: '', password: '', from: '', feedbackTo: '', reportDefaultTo: '',
  });
  ...
  async function save() {
    ...
    try {
      const patch = { ...form };
      // 비밀번호 칸을 건드리지 않은 경우 서버로 보내지 않아 기존 값 유지
      if (!pwTouched) delete patch.password;
      const r = await api.saveMailSettingsApi(patch);
      setStored(r.stored);
      setMsg({ type: 'ok', text: '✅ 저장 완료. SMTP transporter 가 새 설정으로 다시 초기화되었습니다.' });
  ...
  async function sendTest() {
    ...
    try {
      ...
      setMsg({ type: 'err', text: `❌ 테스트 메일 발송 실패: ${e.message || e}${detail}` });
    } finally {
      setBusy('');
    }
  }
    ...
    <div>
      ...
      <div style={S.statusCard}>
        ...
        {active ? (
          <div style={S.statusValueOk}>
            ✅ 활성 — <strong>{active.host}:{active.port}</strong>
            {active.secure ? ' (TLS)' : ''} · 출처: <strong>{active.source === 'ui' ? '관리자 화면' : '환경변수(.env)'}</strong>
          </div>
        ) : (
          <div style={S.statusValueOff}>
            ⚠️ 비활성 — SMTP 가 설정되지 않아 메일 발송이 동작하지 않습니다.
      ...
      <div style={S.panel}>
        ...
        <label style={S.toggleRow}>
          ...
          <span>secure (SSL/TLS) — 포트 465 사용 시 ON</span>
        </label>
      ...
      <div style={S.panel}>
        ...
        <div style={S.testRow}>
          <input type="email" placeholder="테스트 받을 이메일 주소"
            ...
            style={S.testInp} />
          <button onClick={sendTest} disabled={busy === 'test' || !testTo} style={S.testBtn}>
            {busy === 'test' ? '⏳ 발송 중…' : '📤 테스트 보내기'}
          </button>
        </div>
        <div style={S.tip}>
          💡 저장 후 테스트하세요. 실패 시 인증/포트/TLS/네트워크 오류 원인이 표시됩니다.
        </div>
      </div>
